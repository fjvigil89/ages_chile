...
class NonMovingStock(models.TransientModel):
    ...
    def print_report(self):
        ...
        customer_location_ids = LocationObj.search([('usage', '=', 'customer')]).ids
        for each in warehouse_ids_list:
            location_ids = LocationObj.search([('usage', '=', 'internal'), ('location_id', 'child_of',
        ...
        if self.report_type == 'pdf':
            if self.env.context.get('send_mail'):
                ...
                attachment_id = self.env['ir.attachment'].sudo().create(
                    {'datas': values, 'name': 'Non Moving Stock Report', 'datas_fname': 'Non Moving Stock.pdf'})
                return attachment_id
            else:
                final_non_moving_data.update({'break': True})
                vals_new = {'products': final_non_moving_data}
                return self.env.ref('flexipharmacy.non_moving_stock_report').report_action(self, data=vals_new)
            ...
            worksheet.write_merge(0, 1, 0, 5, 'Non Moving Stock Report', style=stylePC)
                ...
                for j in vals:
                    ...
                    for i in range(len(list1)):
                        if list1[i] in ['Cost Price', 'Sale Price']:
                            worksheet.write(row, i, list1[i] + "(" + self.env.user.company_id.currency_id.symbol + ")",
                                            style1)
                    ...
                    for dict in vals.get(j):
                        ...
                        worksheet.write(row, 3, dict.get('quantity'), horz_style_pc)
                        worksheet.col(4).width = 4500
                        worksheet.write(row, 4, str('%.2f' % (dict.get('unit_price'))) + str(
                            self.env.user.company_id.currency_id.symbol), horz_style_pc)
                        worksheet.col(5).width = 4500
                        worksheet.write(row, 5, str('%.2f' % (dict.get('sell_price'))) + str(
            ...
            attachment_id = self.env['ir.attachment'].sudo().create(
                {'datas': self.data, 'name': self.file_name, 'datas_fname': 'Non Moving Stock.xls'})
            if self.env.context.get('send_mail'):
                return attachment_id
            else:
                return {
