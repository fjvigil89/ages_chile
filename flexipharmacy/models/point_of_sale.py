...
class PosConfig(models.Model):
    ...
    def _get_assigned_user(self):
        ...
        for pos_config in self:
            ...
            if assigned_user:
                user_name = assigned_user.mapped('name')
                pos_config.pos_assigned_user = ", ".join(user_name) or ""
    ...
    restrict_expiry_date_alert = fields.Boolean("Restrict Expiry Date")
    # Customer Display
...
class pos_session(models.Model):
    ...
    def get_total_sales(self):
        ...
        if self:
            for order in self.order_ids:
                if order.state != 'draft':
                    total_price += sum([(line.price_subtotal) for line in order.lines])
        return total_price
