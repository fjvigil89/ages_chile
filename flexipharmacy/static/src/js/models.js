odoo.define('flexipharmacy.models', function (require) {
	...
	models.load_fields("product.brand", ['name']);
...
//        	return [['from_date','<=',current_date],['to_date','>=',current_date],['active','=',true]];
        ...
        loaded: function(self,values){
        	 self.db.add_pack_lot_operation(values);
        },
    });
	...
	models.PosModel = models.PosModel.extend({
        ...
        load_new_doctors: function(){
            var self = this;
            var def  = new $.Deferred();
            var fields = this.partner_fields;
            var domain = [['supplier','=',true],['is_doctor','=',true],['write_date','>',this.db.get_partner_write_date()]];
            rpc.query({
                    model: 'res.partner',
                    method: 'search_read',
                    args: [domain, fields],
                ...
                .then(function(doctors){
                    if (doctors) {   // check if the partners we got were real updates
                        def.resolve();
                    } else {
                        def.reject();
                    }
...
//            lines.original_price = this.get_original_price() || false;
            ...
            var new_attr = {
                ...
                serials: this.get_lot_information(),
                prod_uom: this.get_unit(),
                return_valid_days: this.get_return_valid_days(),
            	is_print: order.get_print_serial(),
            	original_price:this.get_original_price() || false,
            }
            $.extend(lines, new_attr);
            return lines;
        },
