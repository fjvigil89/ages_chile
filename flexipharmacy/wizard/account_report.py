...
class AccountingReport(models.TransientModel):
    ...
    def _print_report(self, data):
        ...
        return self.env.ref('flexipharmacy.action_report_financial').report_action(self, data=data, config=False)
    ...
    def print_xls(self):
        ...
        if self.debit_credit:
            ...
            for each in lines:
                ...
                if each.get('level') != 0:
                    ...
                    worksheet.write_merge(i, i, 1, 1, formatLang(self.env, float(each.get('debit')),
                                                                 currency_obj=self.env.user.company_id.currency_id),
                                          fontbold)
                    worksheet.write_merge(i, i, 2, 2, formatLang(self.env, float(each.get('credit')),
                                                                 currency_obj=self.env.user.company_id.currency_id),
                                          fontbold)
                    worksheet.write_merge(i, i, 3, 3, formatLang(self.env, float(each.get('balance')),
                                                                 currency_obj=self.env.user.company_id.currency_id),
                                          fontbold)
                    i = i + 1
        if not self.enable_filter and not self.debit_credit:
            ...
            for each in lines:
                ...
                if each.get('level') != 0:
                    worksheet.write_merge(i, i, 0, 0, each.get('name'), normal_style)
                    worksheet.write_merge(i, i, 1, 1, formatLang(self.env, float(each.get('balance')),
                                                                 currency_obj=self.env.user.company_id.currency_id),
                                          fontbold)
                    i = i + 1
            ...
            for each in lines:
                if each.get('level') != 0:
                    ...
                    worksheet.write_merge(i, i, 1, 1, formatLang(self.env, float(each.get('balance')),
                                                                 currency_obj=self.env.user.company_id.currency_id),
                                          fontbold)
                    worksheet.write_merge(i, i, 2, 2, formatLang(self.env, float(each.get('balance_cmp')),
                                                                 currency_obj=self.env.user.company_id.currency_id),
                                          fontbold)
                    i = i + 1
