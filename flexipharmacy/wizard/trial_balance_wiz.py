...
class trial_balance_wiz(models.TransientModel):
    ...
    def print_pdf(self, data):
        ...
        return self.env.ref('flexipharmacy.report_trial_balance').report_action(self, data=datas)
    ...
    def print_xls(self):
        ...
        if report_data.get('Accounts'):
            for each in report_data['Accounts']:
                ...
                worksheet.write_merge(i, i, j, j, formatLang(self.env, float(each['debit']),
                                                             currency_obj=self.env.user.company_id.currency_id),
                                      stylePC)
                worksheet.write_merge(i, i, j + 1, j + 1, formatLang(self.env, float(each['credit']),
                                                                     currency_obj=self.env.user.company_id.currency_id),
                                      stylePC)
                worksheet.write_merge(i, i, j + 2, j + 2, formatLang(self.env, float(each['balance']),
                                                                     currency_obj=self.env.user.company_id.currency_id),
                                      stylePC)
                i = i + 1
