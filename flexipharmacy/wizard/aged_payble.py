...
class AccountAgedReceivable(models.TransientModel):
    ...
    def generate_aged_payble(self):
        ...
        return self.env.ref('flexipharmacy.report_aged_payble').report_action(self, data=datas)

    @api.multi
    def generated_aged_payable_xls(self):
        report_obj = self.env['report.flexipharmacy.aged_payble_template']
        styleP = xlwt.XFStyle()
        ...
        for each in accont_moveline:
            ...
            worksheet.write_merge(i, i, 1, 1, formatLang(self.env, float(each['direction']),
                                                         currency_obj=self.env.user.company_id.currency_id), stylePC)
            worksheet.write_merge(i, i, 2, 2, formatLang(self.env, float(each['4']),
                                                         currency_obj=self.env.user.company_id.currency_id), stylePC)
            worksheet.write_merge(i, i, 3, 3, formatLang(self.env, float(each['3']),
                                                         currency_obj=self.env.user.company_id.currency_id), stylePC)
            worksheet.write_merge(i, i, 4, 4, formatLang(self.env, float(each['2']),
                                                         currency_obj=self.env.user.company_id.currency_id), stylePC)
            worksheet.write_merge(i, i, 5, 5, formatLang(self.env, float(each['1']),
                                                         currency_obj=self.env.user.company_id.currency_id), stylePC)
            worksheet.write_merge(i, i, 6, 6, formatLang(self.env, float(each['0']),
                                                         currency_obj=self.env.user.company_id.currency_id), stylePC)
            worksheet.write_merge(i, i, 7, 7, formatLang(self.env, float(each['total']),
                                                         currency_obj=self.env.user.company_id.currency_id), stylePC)
            i = i + 1
