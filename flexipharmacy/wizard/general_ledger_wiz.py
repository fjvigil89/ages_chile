...
class account_general_ledger_wiz(models.TransientModel):
    ...
    def print_xls(self, data):
        ...
        for each in report_data['Accounts']:
            ...
            worksheet.write_merge(i, i, 6, 6, formatLang(self.env, float(each['debit']),
                                                         currency_obj=self.env.user.company_id.currency_id), fontbold)
            worksheet.write_merge(i, i, 7, 7, formatLang(self.env, float(each['credit']),
                                                         currency_obj=self.env.user.company_id.currency_id), fontbold)
            worksheet.write_merge(i, i, 8, 8, formatLang(self.env, float(each['balance']),
                                                         currency_obj=self.env.user.company_id.currency_id), fontbold)
            if self.include_init_balance:
                i = i + 1
                worksheet.write_merge(i, i, 4, 5, 'Initial Balance')
                worksheet.write_merge(i, i, 6, 6, formatLang(self.env, float(each['init_bal'][0]),
                                                             currency_obj=self.env.user.company_id.currency_id))
                worksheet.write_merge(i, i, 7, 7, formatLang(self.env, float(each['init_bal'][1]),
                                                             currency_obj=self.env.user.company_id.currency_id))
                worksheet.write_merge(i, i, 8, 8, formatLang(self.env, float(each['init_bal'][2]),
                                                             currency_obj=self.env.user.company_id.currency_id))
            i = i + 1
            for each_line in each['move_lines']:
                ...
                worksheet.write_merge(i, i, 6, 6, formatLang(self.env, float(each_line['debit']),
                                                             currency_obj=self.env.user.company_id.currency_id),
                                      stylePC)
                worksheet.write_merge(i, i, 7, 7, formatLang(self.env, float(each_line['credit']),
                                                             currency_obj=self.env.user.company_id.currency_id),
                                      stylePC)
                worksheet.write_merge(i, i, 8, 8, formatLang(self.env, float(each_line['balance']),
                                                             currency_obj=self.env.user.company_id.currency_id),
                                      stylePC)
                i = i + 1
