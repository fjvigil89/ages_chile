				...
				if(result){
					for (var key in fields) {
						if(key == 'categ_id'){
							...
							var product_brands = self.pos.db.get_all_brands();
							var prod_brand = _.find(product_brands, function(product_brand) { return product_brand.id == fields[key]});
							if(prod_brand){
								self.product['brand_id'] = [prod_brand.id, prod_brand.name];
							}
...
//	                                    	if(prod && selectedOrder.is_sale_product(prod)){
	                                    	...
	                                    	if(r.serial_nums){
	                                    	    var lot_no_list = r.serial_nums.split(":")[1].split(",")
	                                    	    console.log("\n\n\n>>>lot_no_list>>>",lot_no_list)
	                                    	    self.line[r.id]['lot_nos'] = _.map(lot_no_list,function(each){
	                                    	        if(each.length > 1){
	                                    	            return each.trimStart().split("(")[0]
	                                    	        }else{
	                                    	            return false
	                                    	        }
	                                    	    });
	                                    	    console.log("\n\n\n>>>self.line[lot_nos]>>>",self.line[r.id]['lot_nos'])
	                                    	}
	                                    });
	                                    self.lines = lines;
	                                    self.renderElement();
	    		...
	    		if($('.select_item').length > 0){
		            ...
		            _.each($('.select_item'), function(item){
	            		...
	            		if(input_val > 0 && input_val <= orderline.return_qty){
                            ...
                            if(orderline.lot_nos && orderline.lot_nos.length > 0){
                                ...
                                for(var i=0; i < line.get_quantity(); i++){
                                    if(orderline.lot_nos[i]){
                                        line.pack_lot_lines.models[i].attributes.lot_name = orderline.lot_nos[i]
                                    }
                                }
                            }
        ...
        set_quantity_by_lot: function(lot_lines) {
            ...
            line.set_quantity(valid_lots.length);
        },
	    click_cancel: function(){
...
//Order Summary Report
    ...
    var PackLotLinePopupWidget = PopupWidget.extend({
	    ...
	    select_lot: function(ev) {
            ...
            var order = this.pos.get_order();
            var order_line = order.get_selected_orderline();
            var assign_lot = false;
            if(data && add_qty){
                this.all_serial_lots = [];
                this.selected_lot_serial = [];
                var lot_serials = this.options.serials;
                for(var i=0 ; i < lot_serials.length ; i++){
                    if(lot_serials[i].name == data){
                        if(lot_serials[i] && lot_serials[i].life_date){
                            this.all_serial_lots = _.filter(lot_serials, function(each_lot_serial) {
                                return moment(lot_serials[i].life_date) > moment(each_lot_serial.life_date);
                            });
                        }
                        this.$('.packlot-line-input').each(function(index, el){
                            var lot_name = $(el).val();
                            if(lot_name){
                                self.selected_lot_serial.push(lot_name);
                            }
                        });
                        if(self.selected_lot_serial && self.selected_lot_serial.length > 0 &&
                            this.all_serial_lots && this.all_serial_lots.length > 0){
                            self.all_serial_lots.map(function(each_serial_lot){
                                if(($.inArray(each_serial_lot.name, self.selected_lot_serial) >= 0) ){
                                    self.all_serial_lots = jQuery.grep(self.all_serial_lots, function(value) {
                                        return value.name != each_serial_lot.name;
                                    });
                                } 
                            });
                        }
                        if(self.all_serial_lots && self.all_serial_lots.length > 0){
                            var confirm_lot = confirm("The selected medicine have another lot near to expire, please choose another batch of the medicine or you want to proceed?");
                            if(self.pos.config.restrict_expiry_date_alert){
                                if(confirm_lot){
                                    assign_lot = true;
                                } else{
                                    assign_lot = false;
	    ...
	    click_confirm: function(){
            ...
            rpc.query(params, {async: false}).then(function(vals){
                if(vals){
                	...
                	if($("#pay_amount").prop('checked')){
                        partner.remaining_credit_amount = vals.credit_bal - amount;
                    }else{
                        partner.remaining_credit_amount = amount + vals.credit_bal;
                    }
                    var params = {
                        model: 'res.partner',
                        method: 'write',
                        args: [partner_id,{'remaining_credit_amount' : partner.remaining_credit_amount}],
                    }
                    rpc.query(params, {async: false})
                    .then(function(res){
                    });
                	if (self.pos.config.iface_print_via_proxy) {
                        ...
                        receipt = QWeb.render('CustomAddedCreditXmlReceipt', {
                            ...
                            amount: partner.remaining_credit_amount,
